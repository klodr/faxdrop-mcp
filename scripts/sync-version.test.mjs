...
function writeFixture({
  pkgVersion = "9.9.9",
  pkgName = "@klodr/faxdrop-mcp",
  serverPackages = [{ identifier: "@klodr/faxdrop-mcp", version: "0.0.0" }],
...
describe("syncVersion", () => {
  ...
  it("only updates server.json#packages entries whose identifier matches pkg.name", () => {
    ...
    writeFixture({
      ...
      pkgName: "@klodr/faxdrop-mcp",
      serverPackages: [
        { identifier: "@klodr/faxdrop-mcp", version: "0.0.0" },
        { identifier: "@klodr/faxdrop-mcp-docker", version: "0.5.0" },
      ],
    });
    syncVersion(scratch);
    const server = JSON.parse(readFileSync(join(scratch, "server.json"), "utf8"));
  ...
  it.each([
    ["missing version field", JSON.stringify({ name: "@klodr/faxdrop-mcp" })],
    ["empty version string", JSON.stringify({ name: "@klodr/faxdrop-mcp", version: "" })],
    ["non-string version", JSON.stringify({ name: "@klodr/faxdrop-mcp", version: 42 })],
