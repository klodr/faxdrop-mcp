...
`faxdrop-mcp` is an open-source MCP server maintained by **@klodr** in
...
Search [closed issues](https://github.com/klodr/faxdrop-mcp/issues?q=is%3Aissue+is%3Aclosed)
first — most operational questions (token rotation, fax-number pairing,
PDF/DOCX upload limits) are already answered. If nothing matches, open a
new issue with the **Bug report** template and label it `question`.
