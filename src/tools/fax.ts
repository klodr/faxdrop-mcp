...
export function registerFaxTools(server: McpServer, client: FaxDropClient): void {
  ...
  defineTool(
    ...
    [
      ...
      "",
      "USE WHEN: polling for the outcome of a fax sent via `faxdrop_send_fax`. Status values: `queued` | `sending` | `completed` | `failed` | `partial`.",
      "",
      "DO NOT USE: for faxes sent outside this MCP (no provenance — server returns 404). Once status is `completed`, `failed`, or `partial`, STOP polling — these are terminal.",
      "",
    ...
    async ({ faxId }) => {
      // Anti-poll-storm: terminal statuses (completed/failed/partial) are
