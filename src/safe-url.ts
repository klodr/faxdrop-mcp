 ...
 *   same defense surface as `request-filtering-agent`-style request
 *   gating while keeping the native Node `fetch` API (no `node-fetch`
 *   migration).
 *
 *   Residual: tiny TOCTOU window between this lookup and the actual TCP
 *   connect — DNS could rebind in milliseconds. Documented in
 *   `.github/SECURITY.md`; closing it would require socket-level
